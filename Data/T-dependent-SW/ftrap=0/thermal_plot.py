...
DATA_SETS = {
    "Tman": ("Mantle Temperature", "mantle_tem.pdf", "mantle_tem.ps", 0),
    "Tsurf": ("Surface Temperature", "surface_tem.pdf", "surface_tem.ps", 5),
    "Uplate": ("Plate Velocity", "velocity.pdf", "velocity.ps", 15),
    "Sp": ("Plate Flux", "slab_volume.pdf", "slab_volume.ps", 16),
    "Splume": ("Plume Flux", "plume_volume.pdf", "plume_volume.ps", 17)
}
...
for key, (title, filename, psname, column_index) in DATA_SETS.items():
    ...
    if key == "Uplate":
        plt.yscale("log")
        plt.xlabel("Time (Gyr)",fontsize=35)
        plt.ylabel(f"{xlabel_name} (cm/yr)",fontsize=35)
    elif key == "Tsurf":
        plt.xlabel("Time (Gyr)",fontsize=35)
        plt.ylabel(f"{xlabel_name} (K)",fontsize=35)
        ...
        plt.yscale("log")
        plt.xlabel("Time (Gyr)",fontsize=35)
        plt.ylabel(f"{xlabel_name} (m³/s)",fontsize=35)
        plt.ylim(5e2,7e4)
