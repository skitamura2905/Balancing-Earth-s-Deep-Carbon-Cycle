...
def load_additional_data(file_path="carbon_0.5.dat"):
    ...
    if os.path.exists(file_path):
        ...
        return data[:, 0], data[:, 3] / Vman, data[:, 5]*Aman*1e5/g, data[:, 6], data[:, 7]
...
extra_time, mantle, atm, ocean, plate = load_additional_data()

# 描画対象インデックス（例：total（マントル合計）は -3 番目）
DATA_SETS = {
    "UM C": ("Carbon in Upper Mantle", "upper_mantle.png", "upper_mantle.ps", 0),
    "LM C": ("Carbon in Lower Mantle", "lower_mantle.png", "lower_mantle.ps", 1),
    "ATM C": ("Carbon in Atmosphere", "atmosphere.png", "atmosphere.ps", 3),
    "OC C": ("Carbon in Ocean", "ocean.png", "ocean.ps", 4),
    "P C": ("Carbon in Ocean Crust", "plate.png", "plate.ps", 5),
    "Total C": ("Carbon in Whole mantle", "whole_mantle.png", "whole_mantle.ps", -3),
    "FD": ("Mid-Ocean Ridge Flux", "mid-ocean.png", "mid-ocean.ps", 9),
    "FM": ("Arc Flux", "metamorphic.png", "metamorphic.ps", 8),
    "Fplume": ("Plume Flux", "plume.png", "plume.ps", 10),
    "Ingas_UM": ("Ingassing Flux to Upper Mantle", "ingas_um.png", "ingas_um.ps", 6),
    "Ingas_LM": ("Ingassing Flux to Lower Mantle", "ingas_lm.png", "ingas_lm.ps", 7),
    "Inter": ("Lower to Upper Flux", "inter.png", "inter.ps", 11),
    "Weathering_c": ("Continental Weathering", "weather_c.png", "weather_c.ps", 13),
    "Weathering_s": ("Seafloor Weathering", "weather_s.png", "weather_s.ps", 14),
    "Melt_p": ("Plume Residual", "melt_p.png", "melt_p.ps", 15),
    "Subd": ("Subduction Flux", "subd.png", "subd.ps", -2),
    "Total": ("Total Volcanic Outgassing", "total.png", "total.ps", -1)
}
...
for key, (title, filename, psname, column_index) in DATA_SETS.items():
    ...
    if extra_time is not None and mantle is not None:
        if key == "UM C":
            ...
            ax2.set_ylabel(f"{r'$\mathrm{C_{um}}$'} (kg/m³)", fontsize=56)
            ...
            ax2 = ax1.twinx()
            ax2.plot(extra_time, mantle, linestyle="dashed", color="red", label="Foley (2015)")
            ax2.set_ylabel(f"{r'$\mathrm{C_{lm}}$'} (kg/m³)", fontsize=56)
            y_min, y_max = ax1.get_ylim()
    ...
    if extra_time is not None and atm is not None and key in ["ATM C"]:
        ...
        ax2 = ax1.twinx()
        ax2.set_ylabel(f"{r'$\mathrm{P_{atm}}$'} (bar)", fontsize=56)
        y_min, y_max = ax1.get_ylim()
    ...
    if extra_time is not None and plate is not None and key in ["P C"]:
        ax1.plot(extra_time, plate, linestyle="dashed", color="red", label="Foley (2015)")
    ...
    # x軸の単位を変更  
    if key in ["OC C", "P C", "ATM C", "UM C", "LM C", "Total C"]:
        ax1.set_xlabel("Time (Gyr)",fontsize=56)
        ax1.set_ylabel(f"{xlabel_name} (kg)",fontsize=56)
    else:
        ax1.set_xlabel("Time (Gyr)",fontsize=56)
        ax1.set_ylabel(f"{xlabel_name} (kg/yr)",fontsize=56)
    ...
    if key in ["UM C", "LM C", "ATM C"]:
        ...
        if ax2 is not None:
            ...
            labels += labels2
            # 1つの凡例に統合
            ax1.legend(lines, labels, fontsize=38, loc="best")
    else:
