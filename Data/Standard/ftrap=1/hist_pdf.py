...
def generate_combined_histograms(variables, label_mapping, file_range):
    ...
    # FigureとAxesの作成
    fig, axs = plt.subplots(nrows, ncols, figsize=(ncols * 10, nrows * 8)) # figsizeを調整
    axs = axs.flatten() # 1次元配列に変換
 
    # サブプロット間の余白を調整
    plt.subplots_adjust(left=0.08, right=0.95, top=0.95, bottom=0.08, wspace=0.35, hspace=0.4)
    ...
    for idx, (var_name, column_index) in enumerate(variables.items()):
        ...
        try:
            ...
            ax.text(-0.28, 1.0, f'{label_char}', transform=ax.transAxes,
...
def generate_combined_comparison_histograms(variables, label_mapping, 
    ...
    fig, axs = plt.subplots(nrows, ncols, figsize=(ncols * 10, nrows * 8)) 
    axs = axs.flatten()

    plt.subplots_adjust(left=0.08, right=0.95, top=0.95, bottom=0.08, wspace=0.3, hspace=0.4)
