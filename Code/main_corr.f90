program earth_evolution
  ...
  eta_lm = exp(-(b-rm)*alpha*g0/cp_m)
    ...
    ! Initial setup
    cw_s = 1.4e21
    cc_total = 8.6e20*44.0/12.0 ! Taken from Tajika and Matsui (1992)
    cc_oc = 0.0

    time = 0.0
    Seff = (1.0 + 0.4*(1 - time/(4.6e9*3.15e7)))**(-1)
    tequ = (Seff*1360.0*(1-albedo)/(4*sigma))**0.25 - (1360.0*(1.0-albedo)/(4.0*sigma))**0.25
    tsurf = 285.0 + 4.6*((cc_a/3.3e-4)**0.346 - 1.0) + 2.0*tequ
    tm = 3000.0
    ...
    cc_o = (-(k_carbon*a_e/g0 + 44.0*cw_s/18.0 - cc_s) + sqrt((k_carbon*a_e/g0 + 44.0*cw_s/18.0 - cc_s)**2 + 4.0*44.0*cc_s*cw_s/18.0))/2
    ...
    do it = 1,ntime
        ...
        Seff = (1.0 + 0.4*(1.0 - time/(4.6e9*3.15e7)))**(-1)
        tequ = (Seff*1360.0*(1.0-albedo)/(4.0*sigma))**0.25 - (1360.0*(1-albedo)/(4.0*sigma))**0.25
        tsurf = 285.0 + 4.6*((cc_a/3.3e-4)**0.346 - 1.0) + 2.0*tequ
        ...
        Psat = exp(-18.0*2469.0/r_gas*(1/tsurf - 1.0/273.15))/exp(-18.0*2469.0/r_gas*(1.0/288.0 - 1.0/273.15))
        fws = a_e*0.3*0.08*2500.0*10.0e-3*44.0e-3/32.0e-3/3.15e7
        f_cw = fws*(1.0 - exp(-Wref/fws*(cc_a/3.3e-4)**beta_cw*Psat**0.3*exp(42e3/r_gas*(1.0/288 - 1.0/tsurf))))

        ! Seafloor weathering
        f_sw = (uplate/uplate_ref)*1.75e9*44.0*(cc_a/3.3e-4)**gamma_sw/3.15e7 ! Foley (2015)

        ! Temperature dependent seafloor weathering (Turn on this part) !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        !f_sw = (uplate/uplate_ref)*1.75e9*44.0*(cc_a/3.3e-4)**gamma_sw*exp(Esw/r_gas*(1.0/288 - 1.0/tsurf))/3.15e7 ! Foley (2015)
        ...
        ! Carbon content
        cc_um = max(0.0, cc_um + delta_t*(slab_c*ingas_c + inter_flux_c + flux_um_c - outgas_c/e_erupt + (1.0-ftrap)*melt_trap)/(Vman*(1.0-f_man)))
        cc_lm = max(0.0, cc_lm + delta_t*((1.0-slab_c)*ingas_c - outgas_plume_c - flux_um_c - inter_flux_c + ftrap*melt_trap)/(Vman*f_man))
        cc_oc = max(0.0, cc_oc + (0.5*f_cw + f_sw - ingas_c - meta_c)*delta_t)
        cc_s = max(0.0,cc_total - Vman*((1.0-f_man)*cc_um + f_man*cc_lm) - cc_oc)

        cc_o = (-(k_carbon*a_e/g0 + 44.0*cw_s/18.0 - cc_s) + sqrt((k_carbon*a_e/g0 + 44.0*cw_s/18.0 - cc_s)**2 + 4.0*44.0*cc_s*cw_s/18))/2
        cc_a = (cc_s - cc_o)*g0*1e-5/a_e
    ...
    if (outgas_c*3.15e7 < 5.0 * 44.0 * 1e9 .AND. outgas_c*3.15e7 > 1.0 * 44.0 * 1e9 .AND. &
        meta_c*3.15e7 < 11.5 * 44.0 * 1e9 .AND. meta_c*3.15e7 > 2.3 * 44 * 1e9 .AND. &
        e_erupt_plume*outgas_plume_c*3.15e7 < 3.0 * 44.0 * 1e9 .AND. e_erupt_plume*outgas_plume_c*3.15e7 > 0.12 * 44.0 * 1e9 .AND. &
        tsurf < 288.0*1.01 .AND. tsurf > 288.0*0.99) then
